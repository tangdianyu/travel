  ...
  <div class="detaillist">
    <div class="detaillistnav" :style="'position:'+scrollStyle.position">
      <el-tabs v-model="category" @tab-click="onFilter">
        <el-tab-pane
          v-for="(category,index) in categoryLable"
          :label="categoryLable[index].categoryOption.default"
          :key="categoryLable[index].categoryOption.default"
          :name="categoryLable[index].name"
        >
          <div
            class="categoryOption"
            @click="changeOption(index,value,key,num)"
            v-for="(value,key,num) in categoryLable[index].categoryOption"
            :key="value"
            v-if="num> 0"
            :class="{select:num == categoryLable[index].selectNum}"
          >{{value}}</div>
        </el-tab-pane>
      </el-tabs>
    </div>
          ...
          <router-link :to="{path:'/detail/'+result[index].id,params:{id:result[index].id}}">
            <div class="list-wrap">
              ...
              <div class="list-right">
                ...
                <span class="price">费用：{{result[index].fee_str}}</span>
                <span class="Interest">{{result[index].wisher_count}} 人参加</span>
              </div>
            </div>
          </router-link>
        </li>
...
export default {
  ...
  methods: {
    getData(type, day, status) {
      this.$http
        .jsonp("https://api.douban.com/v2/event/list", {
          params: {
            loc: this.cityEn,
            type: type,
            day_type: day,
...
#tab-categoryFirst:before,
#tab-categorySecond:before {
  content: "";
  width: 1px;
  height: 18px;
  position: absolute;
